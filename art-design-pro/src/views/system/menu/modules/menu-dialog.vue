...
<script setup lang="ts">
  ...
  interface Props {
    ...
    type?: 'menu' | 'button'
    lockType?: boolean
    menuList?: AppRouteRecord[] // 菜单列表（用于父菜单选择）
  }
  ...
  const menuTreeOptions = computed(() => {
    const buildTree = (menus: AppRouteRecord[]): any[] => {
      return menus
        .filter((menu) => !menu.meta?.isAuthButton) // 过滤掉按钮
        .map((menu) => ({
          id: menu.id,
  ...
  const loadFormData = (): void => {
    ...
    if (menuType.value === 'menu') {
      ...
      form.id = row.id || 0
      form.parentId = row.parentId || 0 // 添加 parentId
