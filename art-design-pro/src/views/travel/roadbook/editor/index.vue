      ...
      <ElRow :gutter="20">
        ...
        <ElCol :lg="14" :md="24">
          <div class="editor-left">
            ...
            <!-- 日程时间线（仅在有多天行程时显示） -->
            <!-- <ElCard
              v-if="dayCount > 1 && waypoints.length > 0"
              class="timeline-card"
              shadow="never"
            >
              <template #header>
...
<script setup lang="ts">
  ...
  const handleCoverUpload = async (options: UploadRequestOptions) => {
    try {
      ...
      const fileInfo = await uploadFile(options.file, 'image')
      if (fileInfo?.url) {
        formData.value.coverUrl = fileInfo.url
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      router.push('/travel/travel/roadbook/list')
    } catch (err: any) {
  ...
  const saveRoadbook = async () => {
    // 准备途经点数据
    const waypointData = waypoints.value
      .filter((wp) => wp.name && wp.longitude && wp.latitude)
      .map((wp, index) => ({
        name: wp.name!,
        address: wp.address || '',
    ...
    if (isEdit.value) {
      ...
      await roadbookStore.createRoadbook(requestData)
    }
  }
