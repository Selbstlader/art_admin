...
<script setup lang="ts">
          ...
          formatter: (row: ProcessDefResponse) =>
            h('div', { class: 'table-operation' }, [
              ...
              h(
                ...
                {
                  default: () =>
                    h(ElButton, { size: 'small', style: { marginLeft: '8px' } }, () => [
  ...
  const handleCopy = async (row: ProcessDefResponse) => {
    try {
      await ElMessageBox.confirm(`确定要复制流程"${row.name}"吗？`, '复制确认', {
        type: 'info',
  ...
  const handleDelete = async (row: ProcessDefResponse) => {
    try {
      await ElMessageBox.confirm(`确定要删除流程"${row.name}"吗？此操作不可恢复！`, '删除确认', {
        type: 'warning',
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      })
