        ...
        <div class="card-header">
          ...
          <div class="header-actions">
            <ElButton @click="handleExport"
              ><ElIcon><Download /></ElIcon>导出报告</ElButton
            >
            <ElButton @click="goBack"
              ><ElIcon><Back /></ElIcon>返回</ElButton
            >
          </div>
        </div>
      </template>
      ...
      <div v-if="reportData" class="report-content">
        <div class="report-header">
          <ElDescriptions :column="3" border>
            ...
            <ElDescriptionsItem label="检查类型">{{
              reportData.checkType === 'full' ? '全面检查' : '部分检查'
            }}</ElDescriptionsItem>
            <ElDescriptionsItem label="检查时间">{{ reportData.createdAt }}</ElDescriptionsItem>
            <ElDescriptionsItem label="合规分数">
              <ElTag :type="getScoreType(reportData.overallScore)" size="large"
                >{{ reportData.overallScore.toFixed(1) }} 分</ElTag
              >
            </ElDescriptionsItem>
            <ElDescriptionsItem label="检查状态">
              <ElTag :type="getStatusType(reportData.checkStatus)">{{
                getStatusLabel(reportData.checkStatus)
              }}</ElTag>
            </ElDescriptionsItem>
            <ElDescriptionsItem label="检查规范数"
              >{{ reportData.checkedStandards?.length || 0 }} 项</ElDescriptionsItem
            >
          </ElDescriptions>
        </div>
          ...
          <ElRow :gutter="24">
            <ElCol :span="8">
              <ElCard shadow="hover" class="stat-card success">
                <div class="stat-icon"
                  ><ElIcon size="48"><CircleCheck /></ElIcon
                ></div>
                <div class="stat-info"
                  ><div class="stat-value">{{ reportData.passedItems?.length || 0 }}</div
                  ><div class="stat-label">通过项</div></div
                >
              </ElCard>
            </ElCol>
            <ElCol :span="8">
              <ElCard shadow="hover" class="stat-card danger">
                <div class="stat-icon"
                  ><ElIcon size="48"><CircleClose /></ElIcon
                ></div>
                <div class="stat-info"
                  ><div class="stat-value">{{ reportData.failedItems?.length || 0 }}</div
                  ><div class="stat-label">不合规项</div></div
                >
              </ElCard>
            </ElCol>
            <ElCol :span="8">
              <ElCard shadow="hover" class="stat-card warning">
                <div class="stat-icon"
                  ><ElIcon size="48"><Warning /></ElIcon
                ></div>
                <div class="stat-info"
                  ><div class="stat-value">{{ reportData.suggestions?.length || 0 }}</div
                  ><div class="stat-label">改进建议</div></div
                >
              </ElCard>
            </ElCol>
          </ElRow>
        ...
        <div class="report-section">
          <h3 class="section-title"
            ><ElIcon color="#67c23a"><CircleCheck /></ElIcon>通过项 ({{
              reportData.passedItems?.length || 0
            }})</h3
          >
        ...
        <div class="report-section">
          <h3 class="section-title"
            ><ElIcon color="#f56c6c"><CircleClose /></ElIcon>不合规项 ({{
          ...
          <div v-for="(item, index) in reportData.failedItems" :key="index" class="failed-card">
            <ElCard shadow="never">
              <template #header>
                <div class="failed-header">
                  <span class="failed-title"
                    >[{{ item.standardCode }}] {{ item.standardName }}</span
                  >
              ...
              <ElDescriptions :column="1" border size="small">
                ...
                <ElDescriptionsItem label="违规内容">{{
                  item.violationContent
                }}</ElDescriptionsItem>
                <ElDescriptionsItem label="原始规范">{{
                  item.originalRequirement
                }}</ElDescriptionsItem>
                <ElDescriptionsItem label="修改建议"
                  ><span class="suggestion-text">{{ item.suggestion }}</span></ElDescriptionsItem
                >
              </ElDescriptions>
            </ElCard>
          </div>
        ...
        <div class="report-section">
          <h3 class="section-title"
            ><ElIcon color="#e6a23c"><Warning /></ElIcon>改进建议 ({{
                ...
                <div class="suggestion-content">
                  <div class="suggestion-main">
                    ...
                    <ElTag :type="getPriorityType(item.priority)" size="small"
                      >{{ getPriorityLabel(item.priority) }}优先级</ElTag
                    >
                  </div>
...
<script setup lang="ts">
  ...
  const getPriorityColor = (
    ...
    (({ high: 'danger', medium: 'warning', low: 'info' }) as any)[priority] || 'info'
