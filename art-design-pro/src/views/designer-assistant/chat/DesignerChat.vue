        ...
        <div class="session-section">
          ...
          <div class="session-list">
            ...
            >
              ...
              <ElDropdown
                trigger="click"
                @command="(cmd: string) => handleSessionCommand(cmd, session)"
              >
          ...
          <div class="message-list" ref="messageListRef">
            ...
            <div v-if="loading" class="message-group">
              ...
              <div class="message ai-message">
                ...
                <div class="message-content">
                  <div class="message-text" v-if="streamingAnswer">
                    <span v-html="formatMarkdown(streamingAnswer)"></span>
...
<script setup lang="ts">
  ...
  import { designerChatApi, type ChatSession } from '@/api/designer-chat'
  import { getDesignerProjects, type ProjectResponse } from '@/api/designer-project'

  /*** API响应类型 / API response type ***/
  ...
  /*** 项目列表响应类型 / Project list response type ***/
  ...
  // 初始化 / Initialize
  onMounted(async () => {
    // 从URL参数获取项目ID / Get project ID from URL params
    const projectIdParam = route.query.projectId
    if (projectIdParam) {
      currentProjectId.value = Number(projectIdParam)
    }
    await loadProjects()
    await loadSessions()
  })
  ...
  const handleSend = async () => {
    ...
    // 使用流式请求 / Use streaming request
    await designerChatApi.chatStream(
      {
        query: userQuery,
      ...
      (result) => {
        ...
        if (!currentSessionId.value && result.sessionId) {
  ...
  // 快捷提问 / Quick question - 只填充输入框，不直接发送
  ...
  const handleSessionCommand = async (command: string, session: ChatSession) => {
    if (command === 'delete') {
      try {
        ...
        const res = await designerChatApi.deleteSession(session.id)
        if (res.code === 200) {
      ...
      try {
        ...
        a.href = url
        a.download = res.data.filename || 'chat_history.md'
        a.click()
